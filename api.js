...
export async function getRates() {
  ...
  try {
    const [usd, eur] = await Promise.all([
      fetchJson("https://api.frankfurter.app/latest?from=USD&to=UAH,EUR"),
      fetchJson("https://api.frankfurter.app/latest?from=EUR&to=UAH,USD")
    ]);
    const rates = {
      USD_UAH: usd.rates.UAH || cached.USD_UAH || FALLBACK_RATES.USD_UAH,
      EUR_UAH: eur.rates.UAH || cached.EUR_UAH || FALLBACK_RATES.EUR_UAH,
      EUR_USD: eur.rates.USD || cached.EUR_USD || FALLBACK_RATES.EUR_USD,
