...
export function transactionRow(tx) {
  ...
  return `
    <article class="transaction-row">
      ...
      <b class="${tx.type === "income" ? "positive" : tx.type === "expense" ? "negative" : ""}">${sign}${money(tx.amount, tx.currency)}</b>
    </article>
  `;
}
  ...
  return `
    <section class="hero-card">
      ...
      <div class="capital-mix">
        <span style="width:${(totals.fiatUsd / totals.totalUsd) * 100}%"></span>
        <span style="width:${(totals.cryptoUsd / totals.totalUsd) * 100}%"></span>
      </div>
...
export function cryptoView(state) {
  ...
  return `
    ...
    ${section("Активы", `<div class="crypto-list">${portfolio.rows.map((row) => cryptoRow(row, state)).join("")}</div>`, `<button class="small-button" data-open="crypto" type="button">${icon("plus")}Монета</button>`)}
...
function cryptoRow(row, state) {
  return `
    <article class="crypto-row">
      ...
      <div><strong>${row.name}</strong><small>${row.exchange} · ${number(row.amount, 6)} ${row.symbol}</small></div>
      <canvas class="sparkline" data-spark="${row.coinId}" width="88" height="36"></canvas>
      <div class="right"><b>${money(row.value, "USD")}</b><small>${money(convert(row.value, "USD", "UAH", state.rates), "UAH", true)}</small><em class="${row.pnl >= 0 ? "positive" : "negative"}">${percent(row.pnlPct)}</em></div>
    </article>
  `;
}
